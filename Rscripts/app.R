...
AllNames <- readRDS("names.RDS")
Sig30 <- readRDS("Sig30.RDS")
Sig60 <- readRDS("Sig60.RDS")
Sig90 <- readRDS("Sig90.RDS")
kMeans30 <- readRDS("kMeans30.RDS")
kMeans60 <- readRDS("kMeans60.RDS")
kMeans90 <- readRDS("kMeans90.RDS")
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("DaysOfTime",
                  "Tempo (30, 60, 90) em que está diferencialmente expresso: ",
                  min = 30,
                  max = 90,
                  value = 30,
                  step = 30),
      selectInput("TagPicker",
                  "Isoforma: ",
                  #as.data.frame(AmbosClustersSig$at30, stringsAsFactors=F)$Tag)
                  as.data.frame(AllNames, stringsAsFactors=F))
    ),
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    # Captura da matriz a partir dos centros do kMeans
    mainDF <- as.data.frame(kmeans$centers)
    # Monta o subconjunto com base no número do cluster escolhido
    ...
    ggplot(mainDF) +
      ...
      scale_colour_manual(name="Legenda", values=ScaleColors) +
