...
## parameter.deseq.padj: valor de corte para o p-valor ajustado do DESeq2;
# Recomendado pelo Toninho: 0.5 (50% falsos positivos)
# Meu valor usual: 0.05 (5% falsos positivos)
# Valor admitido no Cana Seca: 0.1 (10% falsos positivos)
...
# Amostras
samples <- read.table("/work/rcsilva/projects/drycane/analysis/01_29_kallisto/samples2.txt",
                      header=T)

# Corte de FC no DESeq2
parameter.lfc.sig <- 02
parameter.deseq.padj <- 0.05
...
files <- file.path(dir, "kallisto", samples$folder, "abundance.h5")
...
results.sig$TvS30D <- subset(results.contrast$TvS30D,
                             (results.contrast$TvS30D$log2FoldChange > parameter.lfc.sig) |
                               results.contrast$TvS30D$log2FoldChange < -parameter.lfc.sig |
                               results.contrast$TvS30D$padj < parameter.deseq.padj )
# Para 60D
results.sig$TvS60D <- subset(results.contrast$TvS60D,
                             (results.contrast$TvS60D$log2FoldChange > parameter.lfc.sig) |
                               results.contrast$TvS60D$log2FoldChange < -parameter.lfc.sig |
                               results.contrast$TvS60D$padj < parameter.deseq.padj )
# Para 90D
results.sig$TvS90D <- subset(results.contrast$TvS90D,
                             (results.contrast$TvS90D$log2FoldChange > parameter.lfc.sig ) |
                               results.contrast$TvS90D$log2FoldChange < -parameter.lfc.sig |
                               results.contrast$TvS90D$padj < parameter.deseq.padj)
...
## PS: o kMeans acabou não sendo utilizado, apesar de feito!
# Montagem de kMeans
...
## DESeq2: obtenção dos zeros para suscetível
# Acessar somente as tolerantes
# assays(dds)$counts[,30:53]
# Faça um subconjunto das amostras de contagem cuja soma
# para os suscetíveis é zero
SuscZero <- subset(assays(dds)$counts, rowSums(assays(dds)$counts[,1:29]) == 0)

# para os suscetíveis é zero
ToleZero <- subset(assays(dds)$counts, rowSums(assays(dds)$counts[,30:53]) == 0)

## DEs para zero tolerante
ExclSusc30 <- subset(Sig30, Sig30$Tag == rownames(TolZero))
ExclSusc60 <- subset(Sig60, Sig60$Tag == rownames(TolZero))
ExclSusc90 <- subset(Sig90, Sig90$Tag == rownames(TolZero))

# Criando as tabelas exlcusivas
ExclTole <- SuscZero
ExclSusc <- ToleZero

# Exportar os excel para a pasta de trabalho atual
write.table(Sig30, "./sig30.tsv", sep="\t")
write.table(Sig60, "./sig60.tsv", sep="\t")
write.table(Sig90, "./sig90.tsv", sep="\t")
write.table(ExclTole, "./excl-tole.tsv", sep="\t")
write.table(ExclSusc, "./excl-susc.tsv", sep="\t")

# Gera os mais expressos em relação à um período (tolerante)
tail(sort(results.contrast$TvS30D$log2FoldChange), 10)
mostExpressed30D <- subset(results.contrast$TvS30D, log2FoldChange > 7.8)
write.table(mostExpressed30D, "./ME30D.tsv", sep="\t")

tail(sort(results.contrast$TvS60D$log2FoldChange), 10)
mostExpressed60D <- subset(results.contrast$TvS60D, log2FoldChange > 7.8)
write.table(mostExpressed60D, "./ME60D.tsv", sep="\t")

tail(sort(results.contrast$TvS90D$log2FoldChange), 10)
mostExpressed90D <- subset(results.contrast$TvS90D, log2FoldChange > 7.29)
write.table(mostExpressed90D, "./ME90D.tsv", sep="\t")
